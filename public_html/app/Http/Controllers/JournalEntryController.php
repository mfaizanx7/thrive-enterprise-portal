...
class JournalEntryController extends Controller
{
    ...
    public function index()
    {
        if (\Auth::user()->can('manage journal entry')) {
            $journalEntries = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','JV')->get();
    ...
    public function journalNumber()
    {
        ...
        $latest = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','JV')->latest()->first();
        if (!$latest) {
            return 1;
        }
