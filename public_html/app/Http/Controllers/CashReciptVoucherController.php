...
class CashReciptVoucherController extends Controller
{
    ...
    public function edit($journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            ...
            $subAccounts->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
            $subAccounts = $subAccounts->get()->toArray();
            return view('cash-recipt-voucher.edit', compact('chartAccounts', 'journalEntry' , 'subAccounts'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    function CRVNumber()
    {
        ...
        $latest = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','CRV')->latest()->first();
        if(!$latest)
        {
            return 1;
        }
