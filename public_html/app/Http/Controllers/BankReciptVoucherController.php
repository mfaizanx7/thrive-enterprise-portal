...
class BankReciptVoucherController extends Controller
{
    ...
    public function edit($journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            ...
            $subAccounts->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
            $subAccounts = $subAccounts->get()->toArray();
            return view('bank-recipt-voucher.edit', compact('chartAccounts', 'journalEntry' , 'subAccounts'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function destroy($journalEntry)
    {
        ...
        if(\Auth::user()->can('delete journal entry'))
        {
            ...
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                TransactionLines::where('reference_id', $journalEntry->id)->where('reference', 'BRV')->delete();
    ...
    function BRVNumber()
    {
        ...
        $latest = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','BRV')->latest()->first();
        if(!$latest)
        {
            return 1;
        }
