...
{ 
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            return redirect()->route('bank-payment-voucher.index')->with('success', __('Bank Payment Voucher successfully created.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function edit($journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            ...
            return view('bank-payment-voucher.edit', compact('chartAccounts', 'journalEntry' , 'subAccounts'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function destroy($journalEntry)
    {
        ...
        if(\Auth::user()->can('delete journal entry'))
        {
            ...
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                TransactionLines::where('reference_id', $journalEntry->id)->where('reference', 'BPV')->delete();
    ...
    function BPVNumber()
    {
        ...
        $latest = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','BPV')->latest()->first();
        if (!$latest) {
            return 1;
        }
        if(!$latest)
