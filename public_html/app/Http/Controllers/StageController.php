...
class StageController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->can('manage stage'))
        {
            if (\Auth::user()->type == 'company') {
                ...
                $stages    = Stage::select('stages.*', 'pipelines.name as pipeline')->join('pipelines', 'pipelines.id', '=', 'stages.pipeline_id')->where('pipelines.owned_by', '=', \Auth::user()->ownedId())->where('stages.owned_by', '=', \Auth::user()->ownedId())->orderBy('stages.pipeline_id')->orderBy('stages.order')->get();
            }
            $pipelines = [];
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create stage'))
        {
            ...
            $stage->owned_by = \Auth::user()->ownedId();
            $stage->save();
            Utility::makeActivityLog(\Auth::user()->id,'Stage',$stage->id,'Create Stage',$stage->name);
    ...
    public function update(Request $request, Stage $stage)
    {
        if(\Auth::user()->can('edit stage'))
        {
            ...
            if($stage->created_by == \Auth::user()->ownerId())
            {
                ...
                $stage->pipeline_id = $request->pipeline_id;
                $stage->save();
                Utility::makeActivityLog(\Auth::user()->id,'Stage',$stage->id,'Update Stage',$stage->name);
    ...
    public function destroy(Stage $stage)
    {
        if(\Auth::user()->can('delete stage'))
        {
            if($stage->created_by == \Auth::user()->ownerId())
            {
                ...
                if($deals == 0)
                {
                    Utility::makeActivityLog(\Auth::user()->id,'Stage',$stage->id,'Deal Stage',$stage->name);
                    $stage->delete();
