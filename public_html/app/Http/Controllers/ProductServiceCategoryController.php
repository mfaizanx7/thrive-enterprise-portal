...
class ProductServiceCategoryController extends Controller
{
    public function index()
    {
        if (\Auth::user()->can('manage constant category')) {
            ...
            $categories = ProductServiceCategory::where($column, '=',$ownerId)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('edit constant category')) {
            ...
            if ($category->created_by == \Auth::user()->creatorId()) {
                ...
                $category->chart_account_id = !empty($request->chart_account) ? $request->chart_account : 0;
                $category->save();
                Utility::makeActivityLog(\Auth::user()->id,'Product service Category',$category->id,'Update Product service Category',$category->name);
    ...
    public function destroy($id)
    {
        if (\Auth::user()->can('delete constant category')) {
            ...
            if ($category->created_by == \Auth::user()->creatorId()) {
                ...
                Utility::makeActivityLog(\Auth::user()->id,'Product service Category',$category->id,'Delete Product service Category',$category->name);
                $category->delete();
