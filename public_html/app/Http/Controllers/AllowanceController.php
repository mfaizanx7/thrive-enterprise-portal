...
class AllowanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create allowance'))
        {
            ...
            $allowance->save();
            Utility::makeActivityLog(\Auth::user()->id,'Allowance',$allowance->id,'Create Allowance',$allowance->name);
    ...
    public function update(Request $request, Allowance $allowance)
    {
        if(\Auth::user()->can('edit allowance'))
        {
            if($allowance->created_by == \Auth::user()->creatorId())
            {
                ...
                $allowance->save();
                // here wiill be log 
                Utility::makeActivityLog(\Auth::user()->id,'Allowance',$allowance->id,'Update Allowance',$allowance->name);
    ...
    public function destroy(Allowance $allowance)
    {
        ...
        if(\Auth::user()->can('delete allowance'))
        {
            if($allowance->created_by == \Auth::user()->creatorId())
            {
                Utility::makeActivityLogged(\Auth::user()->id,'Allowance',$allowance->id,'delete Allowance',$allowance->name);
                $allowance->delete();
