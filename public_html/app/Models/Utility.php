...
//                    dd(\Storage::disk(),$path);
    ...
    public static function makeNotification($user_id,$type,$data,$data_id,$action_type){
        ...
        {
            $branch = SpaceType::where('name' , 'Virtual Office')->where('created_by' , $created_by)->first();
            $tax = Tax::where('created_by',$created_by)->where('name','gst')->orderBy('id','Desc')->first();
            if($tax){
                $tax             = new Tax();
                $tax->name       = 'gst';
                $tax->rate       = '10';
            ...
            $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name', 'Virtual Office')->first();
            if($account){}{
                $account              = new ChartOfAccount();
                $account->name        = $branch->name;
                $account->code        = $branch->id;
        ...
        public static function services_branch($user,$created_by)
        {
            ...
            $tax = Tax::where('created_by',$created_by)->where('name','service gst')->orderBy('id','Desc')->first();
            if($tax){}{
                $tax             = new Tax();
                $tax->name       = 'service gst';
                $tax->rate       = '10';
            ...
            $types = ChartOfAccountType::where('owned_by', '=', $created_by)->where('name','Income')->first();
            if($types){
                $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name','Sales Revenue')->first();
            }
            $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name', 'Service Charges')->where('owned_by', '=', $created_by)->first();
            if($account){}{
            $account              = new ChartOfAccount();
            $account->name        = 'Service Charges';
            $account->code        = '00'.$tax->id;
        ...
        public static function security_services($user)
        {
            ...
            $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name', 'Security Deposit')->first();
            if($account){}{
                $account              = new ChartOfAccount();
    ...
    public static function jrentry($data)
    {
        ...
        DB::beginTransaction();
            try {
                $latest = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','JV')->orderBY('id','Desc')->latest()->first();
                if(!$latest)
                {
                    $latest = 1;
                }else{
                ...
                for($i = 0; $i < count($data['items']); $i++)
                {
                    ...
                    $journalItem->credit       = (($data['items'][$i]['quantity'] * $data['items'][$i]['price'])- $data['items'][$i]['discount']) + ($data['items'][$i]['itemTaxPrice']);
                    $journalItem->debit        =  0;
                    $journalItem->save();
                ...
                if($account){
                    ...
                    $journalItem->description = 'Reciveable';
    ...
    {
        $latest = JournalEntry::where('created_by', '=', $data['created_by'])->where('voucher_type','BRV')->orderBY('id','Desc')->latest()->first();
        if(!$latest)
        {
            $latest = 1;
        }else{
    ...
    {
        $latest = JournalEntry::where('created_by', '=', $data['created_by'])->where('voucher_type','CRV')->orderBY('id','Desc')->latest()->first();
        if(!$latest)
        {
            $latest = 1;
        }else{
    ...
    {
        // dd($data);
        $latest = JournalEntry::where('created_by', '=', $data['created_by'])->where('voucher_type', 'BPV')->orderBY('id','Desc')->latest()->first();
        if (!$latest) {
            $latest = 1;
        } else {
            $latest = $latest->journal_id + 1;
    ...
    {
        // dd($data);
        $latest = JournalEntry::where('created_by', '=', $data['created_by'])->where('voucher_type', 'CPV')->orderBY('id','Desc')->latest()->first();
        if (!$latest) {
            $latest = 1;
        } else {
            $latest = $latest->journal_id + 1;
    ...
    {
        $latest = JournalEntry::where('created_by', '=', $data['created_by'])->where('voucher_type', 'JV')->orderBY('id','Desc')->latest()->first();
        if (!$latest) {
            $latest = 1;
        } else {
            $latest = $latest->journal_id + 1;
        ...
        $journal->date = $data['date'];
        $journal->reference = $data['reference'];
        $journal->description = 'Expanse No : ' . @$data['no'];
        $journal->reference_id = $data['id'];
        $journal->category = $data['category'];
        $journal->voucher_type = 'JV';
        // $journal->prod_id       =  $data['prod_id'];
        ...
        //     }
        //     if($account_tax){}else{
        //         $account_tax              = new ChartOfAccount();
        //         $account_tax->name        = 'TAX';
        //         $account_tax->code        = '0000';
        //         $account_tax->type        = $types_t->id;
        //         $account_tax->sub_type    = $sub_type_t->id;
        //         $account_tax->description = 'TAX';
        //         $account_tax->is_enabled  =  1;
        //         $account_tax->created_by  = \Auth::user()->creatorId();
        //         $account_tax->save();
        //     }
        //     if($account_tax){
        //         $journalItem              = new JournalItem();
