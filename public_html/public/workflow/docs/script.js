            ...
            class i {
                        ...
                        if ("input" === i.classList[0] || (this.force_first_input && (null != i.closest(".drawflow_content_node") || "drawflow-node" === i.classList[0]))) {
                            ...
                            if (l !== s && !1 !== o) {
                                if (0 === this.container.querySelectorAll(".connection.node_in_" + s + ".node_out_" + l + "." + c + "." + o).length) {
                                    console.log('ifcon', this.drawflow);
                ...
                addNode(e, t, n, i, s, o, l, c, d = !1) {
                    ...
                    for (var m = 0; m < t; m++) {
                        ...
                        e.style.position = "absolute";  // Added style
                        e.style.top = "-10%";          // Added style
                        e.style.left = "50%";          // Added style
                ...
                export(type, id) {
                    ...
                    for (const [key, module] of Object.entries(e.drawflow)) {
                        for (const [nodeId, nodeData] of Object.entries(module.data)) {
                            const nodeElement = document.querySelector(`#node-${nodeId}`);
                            if (nodeElement) {
                                const assignUserInput = nodeElement.querySelector('input[name="assigned-user"]');
                                const appliedConditions = nodeElement.querySelector('input[name="conditions"]');
                                function parseToObjectOrArray(encodedValue) {
                                    let parsedValue = encodedValue.replace(/&quot;/g, '"');
                                ...
                                const conditionsData = parseToObjectOrArray(appliedConditions?.value || "");
                                const assignedUserData = parseToObjectOrArray(assignUserInput?.value || "");
                                console.log(JSON.stringify(conditionsData, null, 2), "Cleaned Conditions");
                                console.log(JSON.stringify(assignedUserData, null, 2), "Cleaned Assigned User");


                                if (assignUserInput) {
                                    const assignUserValue = assignUserInput.value;
                                    nodeData.assignedUser = assignUserValue;
                                }
                                if (appliedConditions) {
                                    const appliedConditionsValue = appliedConditions.value;
                                    nodeData.appliedConditions = appliedConditionsValue;
                                }
                                nodeData.nodeActualId = nodeElement.id;
                                if (type == 'edit') {
                                    nodeData.actionId = nodeElement.querySelector('.title-box section input[name="actionId"]').value;
                                }
                            }
                        }
                    }
                    if (type == 'edit' && id != null) {
                        $.ajax({
                            url: workflowUpdateUrl,
                            type: 'PUT',
                            data: JSON.stringify({ drawflow: e, type: 'crm'}),
                        ...
                        $.ajax({
                            ...
                            error: function (xhr, status, error) {
                                console.error('Error:', error);
                                alert('Failed to save workflow data.');
                            }
                        });
                    }
                    this.dispatch("export", e);
                    return e;
                }
                import(e, t = !0) {
