                        ...
                        @foreach($home_data['project_status'] as $status => $val)
                            <div class="col-md-6 col-sm-6 mb-5">
                                <div class="align-items-start">
                                    <script>
                                    document.documentElement.style.setProperty('--usedColorDark',usedColorDark);
                                    document.documentElement.style.setProperty('--usedColor', usedColor);
                                    document.documentElement.style.setProperty('--usedColorDarker', usedColorDarker);
                                    document.documentElement.style.setProperty('--usedColorLight', usedColorLight);
                                    </script>
                                    <div class="ms-2">
                                        @php
                                            $colorClass = match(\App\Models\Project::$status_color[$status]) {
                                                'info' => 'usedColorDark',
                                                'warning' => 'usedColor',
                                                'success' => 'usedColorDarker',
                                                'danger' => 'usedColorLight',
                                                default => \App\Models\Project::$status_color[$status],
                                            };
                                        @endphp
                                        <p class="text-muted text-sm mb-0">{{ __( \App\Models\Project::$project_status[$status] ) }}</p>
                                        <h3 class="mb-0" style="color: var(--{{ $colorClass }});">{{ $val['total'] }}%</h3>
                                        <div class="progress mb-0">
                                            <div class="progress-bar" style="width: {{ $val['percentage'] }}%; background-color: var(--{{ $colorClass }});"></div>
                                        </div>
                                    </div>
                            ...
                            @if($home_data['due_project']->count() > 0)
                                @foreach($home_data['due_project'] as $due_project)
                                    <tr>
                                        ...
                                        <td class="">
                                            <span class=" status_badge p-2 px-3 rounded badge" style="background-color: var(--usedColorDarker)">{{ __(\App\Models\Project::$project_status[$due_project->status]) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
