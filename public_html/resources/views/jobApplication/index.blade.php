...
@push('script-page')
  ...
  var job       = JSON.parse(data),
      ...
      question  = job.custom_question;

  // SHOW/HIDE applicant fields and toggle their 'required'
  function toggleField(selector, shouldShow) {
    var $el = $(selector);
    $el.toggleClass('d-none', !shouldShow)
       .find('input, select, textarea')
       .prop('required', shouldShow)
       .prop('disabled', !shouldShow);
  }

  toggleField('.gender',   applicant.indexOf("gender")  !== -1);
  toggleField('.dob',      applicant.indexOf("dob")     !== -1);
  toggleField('.country',  applicant.indexOf("country") !== -1);

  toggleField('.profile',  visibility.indexOf("profile") !== -1);
  toggleField('.resume',   visibility.indexOf("resume")  !== -1);
  toggleField('.letter',   visibility.indexOf("letter")  !== -1);

  // QUESTIONS: first hide & turn off required on all
  $('.question')
    .addClass('d-none')
    .find('input')
      .prop('required', false)
  ...
  // then show & re‑enable only the ones you need
