...
@section('page-title')
    {{__('Edit Recurring Invoice Template')}}
@endsection
@push('css-page')
    <style>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '#customer_id', function() {
                ...
                var selected_contract = "{{ $template->contract_id }}";
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.data, function(key, value) {
                            var selected = (value.id == selected_contract) ? 'selected' : '';
                            $('#contract_id').append('<option value="' + value.id + '" ' + selected + '>' + value.subject + '</option>');
                        });
                    }
                });
            });
            ...
            $(document).on('change', '#contract_id', function() {
                ...
                if (contract_id && contract_id != "{{ $template->contract_id }}") {
                    if (confirm('Select contract will replace all existing line items. Do you want to proceed?')) {
                        $.ajax({
...
@section('content')
    <div class="row">
        {{ Form::model($template, ['route' => ['recurring-invoices.update', \Crypt::encrypt($template->id)], 'method' => 'PUT', 'class' => 'w-100']) }}
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <ul class="nav nav-tabs wizard-nav mb-4">
                        <li class="nav-item">
                            <a class="nav-link active completed" id="nav-step-1" data-step="1" href="#">1. {{__('General Info')}}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link completed" id="nav-step-2" data-step="2" href="#">2. {{__('Line Items')}}</a>
                        </li>
                        <li class="nav-item">
                    ...
                    <div class="wizard-step active" id="step-1">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                {{ Form::number('invoice_day', null, ['class' => 'form-control', 'required' => 'required', 'min' => 1, 'max' => 31]) }}
                            </div>
                            ...
                            {{-- <div class="form-group col-md-4 mt-4">
                                <div class="form-check form-switch">
                                    {{ Form::checkbox('auto_send', '1', null, ['class' => 'form-check-input', 'id' => 'auto_send']) }}
                                    <label class="form-check-label" for="auto_send">{{__('Auto-send Invoice via Email')}}</label>
                                </div>
                            </div> --}}
                        </div>
                        <div class="text-end">
                            <button type="button" class="btn btn-primary next-step">{{__('Next: Edit Items')}}</button>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @foreach($template->items as $item)
                                            <tr data-repeater-item>
                                                <td>{{ Form::text('description', $item->description, ['class' => 'form-control', 'required' => 'required']) }}</td>
                                                <td>{{ Form::number('quantity', $item->quantity, ['class' => 'form-control quantity', 'required' => 'required', 'min' => 1]) }}</td>
                                                <td>{{ Form::number('price', $item->unit_price, ['class' => 'form-control price', 'required' => 'required', 'step' => '0.01']) }}</td>
                                                <td>
                                                    @php $selectedTaxes = explode(',', $item->tax); @endphp
                                                    <select name="tax" class="form-control select tax-select">
                                                        @foreach($taxes as $taxId => $taxName)
                                                            ...
                                                            <option value="{{$taxId}}" data-rate="{{$tax->rate}}" {{ in_array($taxId, $selectedTaxes) ? 'selected' : '' }}>{{$taxName}} ({{$tax->rate}}%)</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>{{ Form::number('discount', $item->discount, ['class' => 'form-control discount', 'step' => '0.01']) }}</td>
                    ...
                    <div class="wizard-step" id="step-3">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tr><td><strong>{{__('Next Invoice Date')}}:</strong></td> <td id="summary-start">-</td></tr>
                        ...
                        <div class="mt-4 d-flex justify-content-between">
                            <button type="button" class="btn btn-secondary prev-step">{{__('Previous')}}</button>
                            <button type="submit" class="btn btn-success">{{__('Update Template')}}</button>
                        </div>
                    </div>
                </div>
            </div>
