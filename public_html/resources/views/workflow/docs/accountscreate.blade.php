...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Manage Accounts Workflow') }}</li>
@endsection
...
@section('content')
    ...
    <head>
        ...
        <script src="{{ asset('public/workflow/docs/accountsscript.js') }}"></script>
    ...
    <body>
        <div class="wrapper">
            <div class="col tab-content active-content">
                <div class="nav-item dropdown">
                    <li data-toggle="collapse" data-target="#bank" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Bank <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="bank">
                    </ul>
                    <li data-toggle="collapse" data-target="#customer" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Customer <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="customer">
                    </ul>
                    <li data-toggle="collapse" data-target="#supplier" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Supplier <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="supplier">
                    </ul>
                    <li data-toggle="collapse" data-target="#bill" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Bill <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="bill">
                    </ul>
                    <li data-toggle="collapse" data-target="#expense" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Expense <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="expense">
                    </ul>

                    <li data-toggle="collapse" data-target="#revenue" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Revenue <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="revenue">
                    </ul>
                    <li data-toggle="collapse" data-target="#credit_note" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">credit note <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="credit_note">
                    </ul>

                    <li data-toggle="collapse" data-target="#payments" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">Payments <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="payments">
                    </ul>
                    <li data-toggle="collapse" data-target="#debit_note" class="collapsed mt-2">
                        <a href="#" style="display: flex; justify-content:space-between;">debit_note <span><i
                                    class="fa fa-chevron-down"></i></span></a>
                    </li>
                    <ul class="sub-menu collapse" id="debit_note">
                    </ul>
                </div>
            </div>
            <div class="col-right">
        ...
        <script>
            ...
            var bankColumn = {!! json_encode($availableDealColumns) !!};
            //compacted accounts columns are
            // 'bankaccountcolumns',
            // 'banktransfercolumns',
            // 'customercolumns',
            // 'suppliercolumns',
            ...
            // 'paymentscolumns',
            // 'debitnotecolumns'
            // here is variable of js to decode into json
            var bankColumns = {!! json_encode($bankaccountcolumns) !!};
            var bankTransferColumns = {!! json_encode($banktransfercolumns) !!};
            var customerColumns = {!! json_encode($customercolumns) !!};
            var supplierColumns = {!! json_encode($suppliercolumns) !!};
            var billColumns = {!! json_encode($bilcolumns) !!};
            var expenseColumns = {!! json_encode($expensecolumns) !!};
            var revenueColumns = {!! json_encode($revenuecolumns) !!};
            var creditNoteColumns = {!! json_encode($creditnotecolumns) !!};
            var paymentsColumns = {!! json_encode($paymentscolumns) !!};
            var debitNoteColumns = {!! json_encode($debitnotecolumns) !!};
            ...
            function populateDropdowns(dropdownId) {
                ...
                if (dropdownContent) {
                    ...
                    // Define a map of stages with corresponding options for each dropdownId
                    const stagesMap = {
                        bill: [{
                        ...
                        credit_note: [{
                            ...
                            node: 'issue-credit-note',
                        }, ],
                        debit_note: [{
                            icon: 'fa-file-alt',
                            text: 'Issue Debit Note',
                            node: 'issue-debit-note',
                        }, ],
                        payments: [{
                                icon: 'fa-credit-card',
                                text: 'Create Payment',
                    ...
                    const stages = stagesMap[dropdownId] || []; // Fetch relevant stages for the dropdownId

                    // Add items dynamically to the dropdown
                    stages.forEach(stage => {
                        const itemDiv = document.createElement('li');
                        itemDiv.classList.add('drag-drawflow');
                        itemDiv.setAttribute('draggable', 'true');
                        itemDiv.setAttribute('ondragstart', 'drag(event)');
                        itemDiv.setAttribute('data-node', stage.node);
                        itemDiv.innerHTML = `<i class="fa ${stage.icon}"></i><span> ${stage.text}</span>`;
            ...
            document.addEventListener('DOMContentLoaded', () => {
                populateDropdowns('bill');
                populateDropdowns('expense');
                populateDropdowns('bank');
                populateDropdowns('customer');
                populateDropdowns('revenue');
                populateDropdowns('supplier');
                populateDropdowns('credit_note');
                populateDropdowns('debit_note');
                populateDropdowns('payments');
            });
        ...
        <script>
            ...
            function addConditionRow(conditionsContainer, clickedNode) {
                let tabidValue = "";
                const inputNode = clickedNode.querySelector('.title-box input[name="nodeid"]');
                if (!inputNode) {
                    console.error("Node ID input not found in clickedNode.");
                    return;
                }
                tabidValue = inputNode.value;
