...
<script>
    $(document).ready(function() {
       $('#now').on('click', function(event) {
           ...
           $('#exp_type').val('pay_now');
           $(this).closest('form').submit();
       });
       $('#later').on('click', function(event) {
           event.preventDefault();
           $('#exp_type').val('pay_later');
           $(this).closest('form').submit();
       });
...
@section('content')
    <div class="row">
        ...
        <div class="modal-footer">
            ...
            <input type="submit" value="{{__('Pay Now')}}" id="now" style="margin-right: 10px;" class="btn  btn-primary">
            <input type="submit" value="{{__('Pay Later')}}" id="later" class="btn btn-secondary">
        </div>
        {{ Form::close() }}
    </div>
@endsection
