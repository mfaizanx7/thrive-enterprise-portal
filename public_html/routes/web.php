...
use App\Http\Controllers\WorkspaceReportsController;
...
Route::get('/generate-attend', function(){
    $startDate = Carbon::now()->subMonth()->day(13);
    $endDate = Carbon::now()->day(13);
    $employeeIds = range(41, 59);
    ...
    foreach ($employeeIds as $employeeId) {
        ...
        while ($date <= $endDate) {
            ...
            $row = [
                ...
                'status'        => $status,
                'clock_in'      => $status === 'present' ? '09:00:00' : null,
                'clock_out'     => $status === 'present' ? '17:00:00' : null,
                'late'          => $status === 'present' ? '00:00:00' : null,
                'early_leaving' => $status === 'present' ? '00:00:00' : null,
                'overtime'      => $status === 'present' ? '00:30:00' : null,
                'total_rest'    => $status === 'present' ? '00:15:00' : null,
                'created_by'    => 2,
                'owned_by'      => 2,
                'created_at'    => now(),
                'updated_at'    => now(),
    ...
    DB::table('attendance_employees')->insert($data);
    return response()->json(['message' => 'Attendance records inserted successfully!', 'total' => count($data)]);
});
...
Route::group(['middleware' => ['verified']], function () {
    ...
    //workspace reports 
    Route::get('spaces-report', [WorkspaceReportsController::class, 'spacesReport'])->name('spacesReport');
    Route::get('clientspaces-report', [WorkspaceReportsController::class, 'clientspaceReport'])->name('clientspaceReport');
    Route::get('vacant-report', [WorkspaceReportsController::class, 'vacantspaceReport'])->name('vacantspaceReport');
    Route::get('meeting-space-report', [WorkspaceReportsController::class, 'meetingspacereport'])->name('meetingspacereport');
    Route::get('asset-report', [WorkspaceReportsController::class, 'assetreport'])->name('assetreport');
    ...
    Route::group(
        ...
        function () {
            ...
            // Recurring Invoices
            Route::post('recurring-invoices/contracts', [RecurringInvoiceController::class, 'getContracts'])->name('recurring-invoices.contracts');
            Route::post('recurring-invoices/contract-details', [RecurringInvoiceController::class, 'getContractDetails'])->name('recurring-invoices.contract.details');
            Route::get('recurring-invoices/{id}/pause', [RecurringInvoiceController::class, 'pause'])->name('recurring-invoices.pause');
            Route::get('recurring-invoices/{id}/resume', [RecurringInvoiceController::class, 'resume'])->name('recurring-invoices.resume');
    ...
    Route::resource('companies', CompaniesController::class)->middleware(['auth', 'XSS', 'revalidate']);
    ...
    Route::get('/workflow/crm-create', [WorkflowController::class, 'create'])->name(name: 'workflow_crm');
    Route::get('/workflow/detail/{id}', [WorkflowController::class, 'show'])->name(name: 'workflow_show');
    ...
    Route::get('import/leads/excel', [LeadController::class, 'importExcelFile'])->name('leads.import_excel_file');
    Route::post('import/leads/excel', [LeadController::class, 'importExcel'])->name('leads.import_excel');
    ...
    Route::post('create-task', [ProjectTaskController::class, 'task_add'])->name('task-add')->middleware(['auth', 'XSS']);
    Route::get('create-task-qa/{id}', [ProjectTaskController::class, 'task_add_qa'])->name('task_create_qa')->middleware(['auth', 'XSS']);
    ...
    Route::get('srs/project/{id}', [ProjectController::class, 'srsview'])->name('project-srs')->middleware(['auth', 'XSS']);
    Route::post('srs/project/store', [ProjectController::class, 'srsstore'])->name('project-srsstore')->middleware(['auth', 'XSS']);
    Route::get('srs/project/view/{id}', [ProjectController::class, 'srsshow'])->name('project-srsview')->middleware(['auth', 'XSS']);
    Route::resource('projects', ProjectController::class)->middleware(['auth', 'XSS']);
    Route::post('srs/project/task/{id}', [ProjectController::class, 'checklistSrs'])->name('srs.task');
    ...
    // booking module
    Route::get('bookingcalendar/{type}', [BookingController::class, 'calendarView'])->name('booking.calendar')->middleware(['auth', 'XSS']);

    // Route::get('/bookings/index', [BookingController::class, 'index'])->name('bookings.index__01')->middleware(['auth', 'XSS']);
    Route::post('bookingcalendar/get_booking_data', [BookingController::class, 'get_booking_data'])->name('booking.calendar.get_booking_data')->middleware(['auth', 'XSS']);
    ...
    Route::resource('account-assets-client', ClientAssetController::class)->middleware(['auth', 'XSS', 'revalidate']);
...
// ---------------------********************************-----------------------
    ...
    Route::group(
        ...
        function () {
            ...
            Route::post('expense/{id}/payment', [ExpenseController::class, 'createPayment'])->name('expense.payment');
